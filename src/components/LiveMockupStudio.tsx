...
export const LiveMockupStudio: React.FC<LiveMockupStudioProps> = ({ onClose }) => {
  ...
  const presetLogos = [
    ...
    },
    {
      ...
      name: 'Tech Ventures',
      svg: (
        <div className="flex flex-col items-center justify-center p-2 text-center text-[#1f1b64]">
          <div className="w-8 h-8 rounded-lg bg-[#1f1b64] text-white flex items-center justify-center font-black text-sm shadow-sm">
            TV
          </div>
    ...
    },
    {
      ...
      name: 'Mysuru Club',
      svg: (
        <div className="flex flex-col items-center justify-center p-2 text-center text-[#191633]">
          <div className="w-9 h-9 rounded-full border-2 border-[#ff6948] flex items-center justify-center font-bold text-xs">
            MC
          </div>
          <span className="text-[8px] font-black uppercase tracking-wider mt-0.5 text-[#ff6948]">EST. 1994</span>
        </div>
      )
    },
    {
      ...
      svg: (
        <div className="flex flex-col items-center justify-center p-2 text-center text-[#0d5930]">
          <span className="text-[12px] font-black tracking-tighter">INNOVA &apos;26</span>
          <span className="text-[7px] font-bold uppercase tracking-widest bg-[#c5efd4] px-1 rounded">ANNUAL SUMMIT</span>
        </div>
      )
    },
    {
      ...
      svg: (
        <div className="flex flex-col items-center justify-center p-2 text-center text-[#1f1b64]">
          <div className="w-7 h-7 bg-[#ffd84d] rounded-md flex items-center justify-center font-black text-xs text-[#1f1b64]">
            K
          </div>
  ...
  const renderCustomTextEmblem = () => {
    ...
    if (textStyle === 'crest') {
      return (
        <div className="flex flex-col items-center justify-center p-2 text-center bg-white/95 backdrop-blur-xs rounded-xl border border-[#d9d1e2] shadow-md">
          <div className="w-8 h-8 rounded-full border-2 border-[#1f1b64] flex items-center justify-center font-black text-xs text-[#1f1b64]">
            {initials}
          </div>
          <span className="text-[9px] font-black tracking-wider uppercase text-[#191633] mt-1">{customText}</span>
          {customSubtitle && <span className="text-[7px] font-bold text-[#ff6948] uppercase tracking-widest">{customSubtitle}</span>}
        </div>
      );
    }
    ...
    if (textStyle === 'modern') {
      return (
        <div className="flex items-center gap-2 p-2 bg-[#1f1b64] text-white rounded-xl shadow-md border border-[#2b2588]">
          <div className="w-7 h-7 bg-[#ffd84d] rounded-lg text-[#1f1b64] flex items-center justify-center font-black text-xs">
            {initials[0]}
          </div>
          <div className="text-left">
            <span className="text-[9px] font-black tracking-wide block uppercase leading-none">{customText}</span>
            {customSubtitle && <span className="text-[7px] text-[#ffd84d] tracking-widest uppercase">{customSubtitle}</span>}
          </div>
        </div>
      );
    }
    ...
    if (textStyle === 'badge') {
      return (
        <div className="flex flex-col items-center justify-center p-2.5 bg-gradient-to-r from-[#1f1b64] to-[#ff6948] text-white rounded-2xl shadow-md">
    ...
    // Minimal
    return (
      <div className="text-center p-1.5 bg-white/90 backdrop-blur-xs rounded-lg border border-[#d9d1e2] shadow-xs">
        <span className="text-[11px] font-black tracking-widest uppercase text-[#1f1b64]">{customText}</span>
        {customSubtitle && <span className="block text-[7px] font-bold text-[#575268] tracking-widest">{customSubtitle}</span>}
      </div>
    );
  };
...
Emblem Type: ${brandingMode === 'custom-text' ? `Custom Text: "${customText}"` : brandingMode === 'upload' ? 'Uploaded Custom Logo' : `Preset: ${selectedPresetLogo}`}`;
  ...
  return (
    <section id="studio-section" className="py-12 sm:py-20 bg-[#fffaf2]">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        
        {/* Studio Header */}
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-10 pb-6 border-b border-[#d9d1e2]">
          <div>
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-[#1f1b64] text-white text-xs font-black uppercase tracking-wider mb-2">
              <Sparkles size={13} className="text-[#ffd84d]" />
              <span>Interactive Merch Studio · Direct factory rates</span>
            </div>
            <h2 className="font-display text-3xl sm:text-4xl font-black text-[#191633] tracking-tight">
          ...
          <div className="flex items-center gap-2 self-start md:self-auto">
            <button
              ...
              className="py-2 px-3.5 rounded-xl bg-white border border-[#d9d1e2] text-xs font-bold text-[#1f1b64] hover:bg-[#e7e2ff] flex items-center gap-1.5 transition-colors shadow-xs"
            >
            ...
            {onClose && (
              <button
                onClick={onClose}
                className="p-2 rounded-xl bg-white border border-[#d9d1e2] text-[#575268] hover:text-[#191633]"
              >
                <X size={18} />
              </button>
            )}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
          ...
          <div className="lg:col-span-7 bg-white rounded-3xl p-6 sm:p-10 border border-[#d9d1e2] shadow-xl relative overflow-hidden flex flex-col items-center">
            
            {/* Top Toolbar */}
            <div className="w-full flex items-center justify-between pb-4 border-b border-[#ece7f2] text-xs">
              <div className="flex items-center gap-2">
                <span className="w-3 h-3 rounded-full border border-black/20" style={{ backgroundColor: selectedColor.hex }} />
                <span className="font-extrabold text-[#1f1b64] uppercase tracking-wider">
                  {selectedProduct.name} · {selectedColor.name}
                </span>
              </div>
              <span className="bg-[#e7e2ff] text-[#1f1b64] font-black px-2.5 py-0.5 rounded-lg">
                Base: {selectedProduct.basePrice}
              </span>
            </div>

            {/* Canvas Stage */}
            <div className="w-full my-6 flex items-center justify-center bg-gradient-to-b from-[#fffaf2] to-[#f2ede6] rounded-2xl p-4 sm:p-6 relative">
              <div ref={stageRef} className="relative w-full max-w-[440px] aspect-square">
                ...
                >
                  {brandingMode === 'upload' && customLogoUrl ? (
                    ...
                    <div className="shrink-0" style={{ transform: `scale(${artworkWidth / 110})` }}>
                      <div className="w-[110px] bg-white/90 rounded-xl shadow-md border border-[#d9d1e2] p-1">
            ...
            {/* Canvas Actions */}
            <div className="w-full flex flex-wrap items-center justify-between gap-3 pt-4 border-t border-[#ece7f2]">
              <div className="flex items-center gap-2">
                <input
                  type="file"
                  ref={fileInputRef}
                ...
                <button
                  ...
                  className="px-4 py-2 rounded-xl bg-[#f2ede6] hover:bg-[#e7e2ff] text-[#1f1b64] font-bold text-xs flex items-center gap-1.5 transition-colors"
                >
                  <Upload size={14} />
          ...
          <div className="lg:col-span-5 space-y-5">
            ...
            <div className="bg-white rounded-3xl p-5 sm:p-6 border border-[#d9d1e2] shadow-xs space-y-3">
              <label className="text-xs font-black uppercase tracking-wider text-[#575268]">
                  ...
                  return (
                    <button
                      ...
                      className={`p-2 rounded-xl border text-left transition-all flex flex-col items-center ${
                        isSelected
                          ? 'border-[#1f1b64] bg-[#e7e2ff]/50 ring-1 ring-[#1f1b64]'
                          : 'border-[#d9d1e2] bg-[#fffaf2] hover:bg-[#f2ede6]'
                      }`}
                    >
                      <div className="w-12 h-12 flex items-center justify-center overflow-hidden">
                        <MockupProduct kind={item.kind} color={selectedColor.hex} className="w-full h-full" />
                      </div>
                      <span className="text-[11px] font-bold text-[#191633] text-center mt-1 line-clamp-1">
                        {item.name}
                      </span>
                    </button>
                  );
            ...
            <div className="bg-white rounded-3xl p-5 sm:p-6 border border-[#d9d1e2] shadow-xs space-y-3">
              <div className="flex items-center justify-between">
                ...
                <span className="text-xs font-bold text-[#1f1b64]">{selectedColor.name}</span>
              </div>
                  ...
                  <button
                    ...
                    className={`w-9 h-9 rounded-xl border transition-transform ${
                      selectedColor.name === c.name
                        ? 'ring-2 ring-[#1f1b64] scale-110'
                        : 'hover:scale-105 border-[#d9d1e2]'
                    }`}
            ...
            <div className="bg-white rounded-3xl p-5 sm:p-6 border border-[#d9d1e2] shadow-xs space-y-4">
              <div className="flex items-center justify-between">
                <label className="text-xs font-black uppercase tracking-wider text-[#575268]">
                  3. Emblem & Artwork Style
                </label>
                <div className="flex items-center gap-1 bg-[#f2ede6] p-0.5 rounded-lg text-[10px] font-bold">
                  <button
                    onClick={() => setBrandingMode('preset')}
                    className={`px-2 py-1 rounded-md transition-colors ${brandingMode === 'preset' ? 'bg-[#1f1b64] text-white' : 'text-[#575268]'}`}
                  >
                    Presets
                  </button>
                  <button
                    onClick={() => setBrandingMode('custom-text')}
                    className={`px-2 py-1 rounded-md transition-colors ${brandingMode === 'custom-text' ? 'bg-[#1f1b64] text-white' : 'text-[#575268]'}`}
                  >
                    Custom Text
                  </button>
                  {customLogoUrl && (
                    <button
                      ...
                      className={`px-2 py-1 rounded-md transition-colors ${brandingMode === 'upload' ? 'bg-[#1f1b64] text-white' : 'text-[#575268]'}`}
                    >
                      Your Logo
                    </button>
                  )}
              ...
              {/* Custom Text Mode */}
              {brandingMode === 'custom-text' && (
                <div className="space-y-3 bg-[#fffaf2] p-3.5 rounded-2xl border border-[#d9d1e2]">
                  <div>
                    <label className="text-[11px] font-bold text-[#575268] block mb-1">Company / Team Name:</label>
                    <input
                      type="text"
                      value={customText}
                      onChange={(e) => setCustomText(e.target.value)}
                      placeholder="e.g. INFOSYS TECH"
                      className="w-full text-xs p-2 rounded-lg border border-[#d9d1e2] bg-white font-bold text-[#191633] focus:outline-none focus:border-[#1f1b64]"
                    />
                  </div>
                  <div>
                    <label className="text-[11px] font-bold text-[#575268] block mb-1">Tagline / Year (optional):</label>
                    <input
                      type="text"
                      value={customSubtitle}
                      onChange={(e) => setCustomSubtitle(e.target.value)}
                      placeholder="e.g. MYSURU · 2026"
                      className="w-full text-xs p-2 rounded-lg border border-[#d9d1e2] bg-white font-medium text-[#191633] focus:outline-none focus:border-[#1f1b64]"
                    />
                  </div>
                  <div>
                    <label className="text-[11px] font-bold text-[#575268] block mb-1.5">Emblem Badge Layout:</label>
                      ...
                      {(['crest', 'modern', 'badge', 'minimal'] as const).map((style) => (
                        <button
                          ...
                          className={`py-1.5 px-2 rounded-lg border capitalize ${
                            textStyle === style ? 'bg-[#1f1b64] text-white border-[#1f1b64]' : 'bg-white text-[#575268] border-[#d9d1e2]'
                          }`}
                        >
                          {style}
                        </button>
                    ...
                    <button
                      ...
                      className={`p-2 rounded-xl border text-xs font-medium text-left transition-colors ${
                        selectedPresetLogo === pl.id && brandingMode === 'preset'
                          ? 'border-[#1f1b64] bg-[#e7e2ff]/50 text-[#1f1b64] font-bold ring-1 ring-[#1f1b64]'
                          : 'border-[#d9d1e2] bg-[#fffaf2] text-[#575268] hover:bg-[#f2ede6]'
                      }`}
                    >
                      {pl.name}
                    </button>
              ...
              {/* Placement & Scale */}
              <div>
                <label className="text-[11px] font-bold uppercase tracking-wider text-[#575268] block mb-2">
                  Position & Size
                </label>
                ...
                <div className="space-y-1">
                  <div className="flex justify-between text-xs font-bold text-[#575268]">
                  ...
                  <input
                    ...
                    className="w-full accent-[#1f1b64] cursor-pointer"
                  />
                </div>
              </div>
            </div>
